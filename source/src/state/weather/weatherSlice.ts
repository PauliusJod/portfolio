...
import { sortPrec, sortTemp, sortTime, sortWind, WeatherForm } from "../../utils/utils";
...
const weatherSlice = createSlice({
  ...
  reducers: {
    ...
    setTemperature: (state, action: PayloadAction<{ index: number }>) => {
      state.dayTemperature = sortTemp(action.payload.index + 1, state.temperature);
    },
    setPrecipitation: (state, action: PayloadAction<{ index: number }>) => {
      state.dayPrecipitation = sortPrec(action.payload.index + 1, state.precipitation);
    },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchWeatherAsync.fulfilled, (state, action: PayloadAction<any>) => {
        state.value = action.payload;
        const { time, temperature_2m, precipitation_probability, wind_speed_10m } = action.payload.data.hourly;
        state.time = time || [];
...
export const fetchWeatherAsync = createAsyncThunk("weather/fetchWeatherAsync", async ({ weatherForm }: WeatherForm) => {
...
export const { resetWeather, resetDayWeather, setTime, setTemperature, setPrecipitation, setWind } =
